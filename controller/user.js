...
router.get('/logout', async function (req, res) {
  ...
  const username = parseCookie(req.headers.cookie).loginName


  const result = await userModel.findOneAndUpdate({ username }, { temporaryToken: '' })
  if (result) {
    res.send({
      code: 401,
      message: '退出登录'
    ...
    res.status(410).send({ code: 410, error: 'error' })
  }
